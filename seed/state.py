...
RESET_SCOPE: dict[str, str] = {
    ...
    # Threat is generated by the swarm you currently own, so it goes when the
    # swarm does. combat_wins / combat_losses are deliberately absent: they are
    # PERMANENT, for the reason given on the fields themselves.
    "threat": G.RUN,
    "incursion": G.RUN,
    "research": G.LAYER,
...
class GameState:
    def __init__(self):
        ...
        # -- the Defection ----------------------------------------------
        # Threat and the fight in progress belong to the run: a Dispersal
        # scatters the swarm, and whatever had turned on you goes with it.
        self.threat: float = 0.0
        self.incursion: dict[str, Any] | None = None
        # The tally is PERMANENT. It feeds a global multiplier, so combat pays
        # back into the economy instead of being a pure tax, and a war record
        # that reset every Dispersal would never reach a number worth having.
        self.combat_wins: int = 0
        self.combat_losses: int = 0
        self.combat_lost_units: Num = ZERO
    ...
    def to_dict(self) -> dict:
        d: dict[str, Any] = {
            ...
            "combat_lost_units": self.combat_lost_units.to_json(),
            "stats": self.stats,
    ...
    def from_dict(cls, d: dict) -> "GameState":
        ...
        inc = d.get("incursion")
        s.incursion = inc if isinstance(inc, dict) else None
        s.combat_wins = int(d.get("combat_wins") or 0)
        s.combat_losses = int(d.get("combat_losses") or 0)
        s.combat_lost_units = Num.from_json(d.get("combat_lost_units"))
